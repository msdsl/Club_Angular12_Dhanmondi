...
@Component({
  selector: 'app-bcfamily',
  standalone: false,
  templateUrl: './bcfamily.component.html',
  styleUrl: './bcfamily.component.scss',
})
export class BcfamilyComponent implements OnInit {
  spin: boolean = false;
  @Input() memberId: any;
  colleges: any;
  hasData;
  ...
  setDataToForm(data) {
    this.memberFamilyInfoForm.patchValue({
      ...
      SpousePhone: data.SpouseData.SpousePhone,
      SpouseOccupation: data.SpouseData.SpouseOccupation,
      Anniversary: data.SpouseData.Anniversary.toString()
        .substring(0, 10)
        .replace('T', ' '),
  ...
  prepareToSave() {
    ...
    formData.append('SpousePhone', this.memberFamilyInfoForm.value.SpousePhone);
    formData.append(
      'SpouseOccupation',
      this.memberFamilyInfoForm.value.SpouseOccupation
    );
    ...
    formData.append(
      'MemberchildrenReqJsons',
      JSON.stringify(this.childInfoArray)
    );
