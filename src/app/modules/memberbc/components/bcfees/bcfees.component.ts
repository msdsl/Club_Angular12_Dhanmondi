...
@Component({
  selector: 'app-bcfees',
  standalone: false,
  templateUrl: './bcfees.component.html',
  styleUrl: './bcfees.component.scss',
})
export class BcfeesComponent implements OnInit {
  @Input() memberId: any;
  memberFeesData: any;
  ...
  memberShipFees(data: any) {
    if (data.MemberTypeId > 0) {
      this.memberFeesService.getMemberFeesPagination(1, 1000).subscribe(
        (res) => {
          this.memberFeesData = res.Data.filter(
            (c) => c.MemberTypeId == this.member.MemberTypeId
          );
          if (data.Id > 0) {
            this.memberFeesData.forEach(
              (item) =>
                (item.IsChecked = data.MemberFeesMapRess.find(
                  (x) => x.MemberShipFeeId == item.Id
                )?.IsChecked
                  ? true
                  : false)
            );
